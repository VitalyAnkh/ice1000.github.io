...
```cpp
#ifndef HUGE_BETTER_VECTOR_H
#define HUGE_BETTER_VECTOR_H
...
sona::Pair<T *, T *> sona::Vector<T>::allocateRawMemory(SizeType n) {
	if (n == 0) n = 1;
	auto *memoryStartPoint = reinterpret_cast<T *>(::operator new(n * sizeof(T)));
...
sona::Vector<T>::~Vector() {
	for (T *rawIt = mBufBegin; rawIt != mBufEnd; ++rawIt) destroyAt(rawIt);
	deallocateRawMemory(mBufBegin);
}
